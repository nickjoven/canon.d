...
/// the witness is the decimal fraction, recomputed by division — a route that
/// shares nothing with the integer canonicalizer.
fn count_ratio_schema() -> Schema {
    Schema::new("count_ratio", 1)
        ...
        .witness("fraction", FieldKind::Float)
...
fn main() {
    ...
    // 5. WITNESS CHECK — recompute the fraction by an independent route (3/5 by
    //    division) and confirm it matches the sealed witness. The claim schema
    //    declares a witness, so it self-checks rather than going to review.
    let recomputed = json!({ "fraction": 3.0_f64 / 5.0 });
    let witness_ok = s.claim.verify_witness(&cs, &recomputed).expect("verify_witness");
    println!("5. witness        fraction recomputed by division == sealed: {witness_ok}");
    assert!(witness_ok);
    ...
    // A wrong reading is caught here, not trusted: claim 3/5 but sealed witness 0.7.
    let bad = Quantum::seal(
        &cs,
        &json!({"sample":"slide-7","trait":"stained","hits":3,"total":5,
                "grounds":[reading.cid],"fraction":0.7}),
    )
    .expect("seal bad");
    let bad_ok = bad.verify_witness(&cs, &json!({"fraction": 3.0_f64/5.0})).expect("verify");
    println!("   tamper check   claim says 3/5 but witness sealed 0.7 -> verifies: {bad_ok}");
    assert!(!bad_ok);
