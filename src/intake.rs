...
impl DomainRange {
    ...
    pub fn open(lo: Rat, hi: Rat) -> Self {
        ...
        DomainRange { lo, hi, lo_open: true, hi_open: true, label }
    }

    /// The closed range `[lo, hi]`.
    pub fn closed(lo: Rat, hi: Rat) -> Self {
        let label = format!("[{}, {}]", lo.reduced_string(), hi.reduced_string());
        DomainRange { lo, hi, lo_open: false, hi_open: false, label }
    }

    /// Exact membership: `lo (< | ≤) v (< | ≤) hi` per the bound openness.
    pub fn contains(&self, v: Rat) -> bool {
        let above_lo = if self.lo_open { self.lo < v } else { self.lo <= v };
        let below_hi = if self.hi_open { v < self.hi } else { v <= self.hi };
        above_lo && below_hi
    }
...
impl SubjectDomains {
    ...
    pub fn witness(&self, subject: &str, num: i128, den: i128) -> Plausibility {
        ...
        let out_of = || Plausibility::OutOfDomain { domain: format!("{subject} ∈ {}", range.label) };
...
impl Default for PromotionPolicy {
    fn default() -> Self {
        PromotionPolicy { min_corroboration: 1 }
    }
}
...
pub fn cross_audit_routes(
    ...
    report.promoted.sort_by(|a, b| b.corroboration.cmp(&a.corroboration).then(a.value.cmp(&b.value)));
    report.queued.sort_by(|a, b| b.corroboration.cmp(&a.corroboration).then(a.value.cmp(&b.value)));
    report
}
...
fn str_field_of(q: &crate::Quantum, field: &str) -> String {
    q.field(field).and_then(|v| v.as_str()).unwrap_or("").to_string()
}
fn int_field(q: &crate::Quantum, field: &str) -> i128 {
    q.field(field).and_then(|v| v.as_i64()).map(|n| n as i128).unwrap_or(0)
}
...
mod tests {
    ...
    fn domain_witness_rejects_out_of_range_and_accepts_in_range() {
        ...
        assert_eq!(d.witness("omega_lambda", 6847, 10000), Plausibility::InDomain);
        assert!(d.witness("omega_lambda", 12, 1).blocks(), "12 is not a density fraction");
        assert!(d.witness("omega_lambda", 1, 1).blocks(), "exactly 1 is out of the open range");
        // an undeclared subject is unknown, not a violation
        assert_eq!(d.witness("some_other_quantity", 5, 1), Plausibility::NoConstraint);
    }
    ...
    fn the_residual_is_filtered_but_the_real_fact_promotes() {
        ...
        let mut all = Vec::new();
        all.extend(structure_span_with("Ω_Λ = 13/19", "en", &lex(), Route::Prefix).unwrap().structured);
        all.extend(structure_span_with("Ω_Λ = 12", "en", &lex(), Route::Prefix).unwrap().structured);

        let report = cross_audit_routes(&all, &SubjectDomains::harmonics(), &PromotionPolicy::default());
    ...
    fn two_routes_corroborate_one_proposition() {
        ...
        let mut all = Vec::new();
        all.extend(structure_span_with("Ω_Λ = 13/19", "en", &lex(), Route::Prefix).unwrap().structured);
        all.extend(structure_span_with("13/19 = Ω_Λ", "en", &lex(), Route::Postfix).unwrap().structured);
    ...
    fn under_corroboration_queues_even_when_in_domain() {
        let one = structure_span_with("Ω_Λ = 13/19", "en", &lex(), Route::Prefix).unwrap().structured;
        let strict = PromotionPolicy { min_corroboration: 2 };
